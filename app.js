...
process.env.BUILD_HASH = require('./static/build-hash.json');
...
app.use('/images/color', express.static('public/images/color'));
app.use('/images/actions', express.static('public/images/actions'));
app.use('/images/gallery', express.static('public/images/gallery'));
app.use('/images/part', express.static('public/images/part'));
  ...
  data.productMenu.forEach((brand, index1) => {
    ...
    brand.products.forEach((product, index2) => {
      ...
      if (product.iGenerateUriMaterial == 1) {
        ...
        data.brandMenu[index1].products.material[material_index].list.push(
          product_item
        );
      } else {
...
app.post('/admin/ProductUpdate', async (req, res) => {
  var iProductID = req.body.product.iProductID
    ? req.body.product.iProductID
    : false;

  req.body.product.iProductParam1 = req.body.product.iProductParam1
    ? req.body.product.iProductParam1
    : null;
  req.body.product.iProductParam2 = req.body.product.iProductParam2
    ? req.body.product.iProductParam2
    : null;
  req.body.product.iProductParam3 = req.body.product.iProductParam3
    ? req.body.product.iProductParam3
    : null;
  req.body.product.iProductParam4 = req.body.product.iProductParam4
    ? req.body.product.iProductParam4
    : null;
  req.body.product.iProductParam5 = req.body.product.iProductParam5
    ? req.body.product.iProductParam5
    : null;
  req.body.product.iProductParam6 = req.body.product.iProductParam6
    ? req.body.product.iProductParam6
    : null;

  req.body.product.iGenerateUriMaterial = req.body.product.iGenerateUriMaterial
    ? 1
    : 0;
  req.body.product.iGenerateUriBrus = req.body.product.iGenerateUriBrus ? 1 : 0;

  req.body.product.iPrice = req.body.product.iPrice
    ? req.body.product.iPrice
    : null;
  req.body.product.iActive = req.body.product.iActive ? 1 : 0;

  req.body.product.iMaterialCategoryID = req.body.product.iMaterialCategoryID
  ...
  if (
    req.body.product.iGenerateUriMaterial &&
    req.body.product.iMaterialCategoryID
  ) {
  ...
  req.body.product.sProductURI = cyrillicToTranslit()
    .transform(uri_string, '_')
    .toLowerCase();
  // *** //
...
app.post('/admin/BrandUpdate', async (req, res) => {
  ...
  if (brand.sBrandTitle) {
    brand.sBrandURI = cyrillicToTranslit()
      .transform(brand.sBrandTitle, '_')
      .toLowerCase();
  }
...
app.get('/', async (req, res) => {
  ...
  data.description =
    'Просто Окна – замер, проектирование, изготовление и реализация светопрозрачных конструкций любых типов: пластиковые окна ПВХ, алюминиевые окна, деревянные окна из евробруса, витражи, фасады и стоечно-ригельные системы. Монтажные работы любой сложности. Окна от производителя по лучшим ценам в Москве, МО и России. Мы не занимаемся накруткой цены, вы точно знаете, за что платите. Просто Окна – с нами просто.';
  ...
  product.forEach((p) => {
    ...
    if (p.material_category) {
      ...
      let material_category_item = material_item.list.find(function (
        ...
        return (
          material_category.iMaterialCategoryID ==
          p.material_category.iMaterialCategoryID
        );
      });
    ...
    data.product.forEach((x) => {
      if (x.material_category) {
        x.material_category.sMaterialCategoryTitle =
          ...
          x.material_category.sMaterialCategoryTitle.slice(1);
      }
    });
    // res.json(data.product)
...
app.get('/part/:sPartURI', async (req, res) => {
  ...
  if (partBrand)
    return res.redirect(
      '/part/' + part.sPartURI + '/' + partBrand.sPartBrandURI
    );
  res.sendStatus(404);
});
app.get('/part/:sPartURI/:sPartBrandURI', async (req, res) => {
  ...
  if (partModel)
    return res.redirect(
      '/part/' + sPartURI + '/' + sPartBrandURI + '/' + partModel.sPartModelURI
    );
  res.sendStatus(404);
});
...
app.post('/send', async (req, res) => {
  ...
  upload(req, res, function (err, responce) {
    ...
    var tel =
      req.body.tel && req.body.tel != 'undefined' ? req.body.tel : false;
    var email =
      req.body.email && req.body.email != 'undefined' ? req.body.email : false;
    var from =
      req.body.from && req.body.from != 'undefined' ? req.body.from : false;
    ...
    var message =
      req.body.message && req.body.message != 'undefined'
        ...
        : false;
    var page_path =
      req.body.page_path && req.body.page_path != 'undefined'
        ? req.body.page_path
        : false;
    ...
    var data = {
      ...
      to:
        '<davydova.o@prostokna.ru>, <prosto.pochta2013@mail.ru>, <viki.z@prostokna.ru>',
