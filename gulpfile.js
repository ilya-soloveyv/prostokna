...
gulp.task('rev', (done) => {
  gulp
    .src(['public/app.min.css', 'public/app.min.js'])
    .pipe(rev())
    .pipe(gulp.dest('public/'))
    .pipe(rev.manifest())
...
gulp.task('rev_collector', (done) => {
  gulp
    .src(['public/manifest/**/*.json', 'views/layouts/app.pug'])
    .pipe(
      revCollector({
        replaceReved: true,
      })
...
gulp.task(
  'production',
  gulp.series('js_min', 'css_min', 'rev', 'rev_collector', 'rev_clean')
);

gulp.task('pug', function buildHTML() {
  return gulp.src('views/index/s4.pug').pipe(pug()).pipe(gulp.dest('dist'));
});
...
gulp.task('sass_admin', function () {
  return gulp
    .src(['public/src/admin/sass/**/*.scss'])
    .pipe(sass())
...
gulp.task(
  ...
  /* ['nodemon'], */ function () {
    browserSync.init(null, {
      ...
      files: ['views/**/*.*', 'public/src/css/**/*.*', 'public/src/js/**/*.*'],
      port: 7000,
    });
  }
);
