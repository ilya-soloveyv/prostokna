  ...
  methods: {
    get: function() {
      if (this.iPartBrandID) {
        axios.post('/admin/part/getPartColorList', {
    ...
    destroy(iPartColorID) {
      if(confirm("Подтверждаете удаление?")) {
        axios.post('/admin/part/destroyPartColor', { iPartColorID }).then((responce) => {
          this.get()
          if (iPartColorID == this.iPartColorID) this.$router.push('/part/' + this.iPartID + '/' + this.iPartBrandID)
        })
      }
    }
  },
