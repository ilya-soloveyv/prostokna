...
function initGroupPreview() {
  ...
  $('.js-group_preview').each(function(){
    var $previews = $(this);
    var $items = $previews.find('li.item');
    var $items_links = $items.find('div.link');

    $items_links.click(function(){
      $previews.parents('.tab').find('.js-preview_desc').html( $(this).find('.text').html().replace(/<br>/g,"") );
		  setCurWid($(this).attr('data-wid'));
      isSelectSubWindow = true;
    });

    $items.click(function(){
      $previews.parents('.tab').find('.js-preview_img').attr('src',$(this).attr('data-preview'));
      $previews.parents('.tab').find('.js-preview_desc').html($(this).attr('data-desc'));
