...
function flow_sidebar_filter($default_sidebar){
	...
	$post_ID_to_use = $wp_query->post->ID;
	if(is_home()){
...
function flow_sidebar_about_meta_box(){ ?>
	...
	<p>This is the code that you should put directly in template files to be able to access your sidebar and to see it on "Widget Areas Replacement" list under [Pages > Add New], [Posts > Add New] etc.: <code>&lt;?php dynamic_sidebar( apply_filters( 'flow_sidebar', 'flow-sidebar-ID' ) ); ?&gt;</code></p>
	<p>If you wish to use a widget area directly on a page or in a post, please use this shortcode: <code>[flow-sidebar id="flow-sidebar-ID"]</code></p>
	<?php
}
			...
			echo '<div class="inside">';
				echo '<div class="flow_sidebars_container">';
					...
					if(is_array($sidebars) && !empty($sidebars) && count($sidebars) >= 2){
						echo '<p>This page may contain some widget areas (but perhaps not all widget areas listed below are used on this page). You can replace existing widget areas with some other widget areas below (leave "None" to use the default widget area). Double replacement (e.g. [Sidebar -> Sidebar 1] and [Sidebar 1 -> Sidebar 2]) won\'t take effect - you need to set both to "Sidebar 2" to achieve what you\'re looking for. You can create more <a href="'.get_admin_url().'themes.php?page=flow_sidebars" target="_blank">Widget Areas</a> and add them some <a href="'.get_admin_url().'widgets.php" target="_blank">Widgets</a>.</p>';
						echo '<ul>';
						$i = 0;
