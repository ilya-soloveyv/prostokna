  ...
  methods: {
    ...
    destroy: function(iPartBrandID) {
      if (confirm('Подтверждаете удаление?')) {
        axios
          .post('/admin/part/destroyPartBrand', { iPartBrandID })
          .then(responce => {
            this.get();
            if (iPartBrandID == this.iPartBrandID)
              this.$router.push('/part/' + this.iPartID);
          });
      }
    }
  },
