import Vue from 'vue';
import '@scss/intuitive/intuitive.scss';
...
if ($('#intuitive').length) {
  function positionButtons() {
    ...
    $('#intuitive .intuitive-wrap .buttons')
      .height(buttons_width)
      .css({ 'margin-top': -(buttons_width / 2) + 'px' });
    var num = 10;
    ...
    for (i = 0; i < num; i++) {
      ...
      if (i == 5) {
        $('#intuitive .intuitive-wrap .buttons .button_1').css({
          top: top,
          left: left,
        });
      }
      if (i == 6) {
        $('#intuitive .intuitive-wrap .buttons .button_2').css({
          top: top,
          left: left,
        });
      }
      if (i == 7) {
        $('#intuitive .intuitive-wrap .buttons .button_3').css({
          top: top,
          left: left,
        });
      }
  ...
  var vmIntuitive = new Vue({
    ...
    methods: {
          ...
          item.forEach(function (item2, i2) {
            ...
            item2.answers.forEach(function (item3, i3) {
              if (item3.status === true) {
                Vue.set(
                  vmIntuitive.questions[i][i2].answers[i3],
                  'status',
                  false
                );
              }
      ...
      useQuestion: function (index) {
        if (
          !this.questions[this.currentSubject][index].next ||
          (this.questions[this.currentSubject][index - 1] &&
            !this.questions[this.currentSubject][index - 1].next)
        ) {
      ...
      useAnswer: function (index) {
        this.selectionResult(
          this.questions[this.currentSubject][this.currentQuestion].answers[
            index
          ].windows
        );

        var temp = [];
        this.questions[this.currentSubject][
        ...
        if (this.findInArray(temp, true) != '-1') {
          Vue.set(
            vmIntuitive.questions[this.currentSubject][this.currentQuestion],
            'next',
            false
          );
        } else {
      ...
      nextQuestion: function () {
        if (!this.questions[this.currentSubject][this.currentQuestion].next) {
          if (
            this.questions[this.currentSubject].length ==
