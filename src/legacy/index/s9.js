import '@scss/index/s9.scss';

if (document.querySelector('.layout-index')) {
  if (document.body.clientWidth < 575) {
    $('.s9-nav > ul').slick({
      nextArrow:
        '<button type="button" class="slick-arrow slick-next" aria-label="Следующий"><svg aria-hidden="true"><use xlink:href="#arrow-next-a" /></svg></button>',
      prevArrow:
        '<button type="button" class="slick-arrow slick-prev" aria-label="Предыдущий"><svg aria-hidden="true"><use xlink:href="#arrow-prev-a" /></svg></button>',
    });
  }
}
...
if ($('#map').length) {
  ...
  script.src =
    'https://maps.googleapis.com/maps/api/js?key=AIzaSyCt8zCnAdWM57-hf6vndvzWZ0UKDPc2yxE&callback=initMap';
