...
if (document.querySelector('.layout-index')) {
  if (document.body.clientWidth > 910) {
    ...
    const s2CloseCardBtns = document.querySelectorAll(
      '.s2-front-card .card-close'
    );
    const s2BackCardBtns = document.querySelectorAll(
      '.s2-front-card .card-back'
    );
    ...
    s2Cards.forEach((card, index) => {
      card.addEventListener('click', (event) => {
        ...
        let backCardTarget = document.querySelectorAll('.s2-back-card-wrap')[
          index
        ];
        ...
        if (
          backCardTarget.offsetHeight <
          backCardTarget.querySelector('.s2-back-card').scrollHeight
        ) {
...
function showLowerLevelsLists(bool) {
  let backBtn =
    document.querySelector('.s2-card.active .card-back') ||
    document.querySelector('.s2-back');
  let activeBackCard =
    document.querySelector('.s2-back-card-wrap.isActive') ||
    document.querySelector('.s2-back-card-wrap.slick-current');
