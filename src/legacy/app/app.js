...
$(document)
  .on('mouseenter', '#top .l .menu', function () {
...
$(document)
  .on('mouseenter', '#fullpage_nav ul', function () {
...
$('#zay_modal')
  ...
  .on('hidden.bs.modal', function (e) {
    $('#zay_modal')
      .removeClass('zay_modal_black')
      .removeClass('zay_modal_white');
  });
...
$('#modal_gallery_owl')
  ...
  .on('hide.bs.modal', function (event) {
    ...
    $('#product .toggle_submenu .bg .icons .hamburger').removeClass(
      'is-active'
    );
...
const cookie_modal_scammers = getCookie('modal_scammers');
if (!cookie_modal_scammers) {
  $('#modal_scammers')
    .modal()
    .on('hide.bs.modal', function (event) {
      setCookie('modal_scammers', true, { 'max-age': 3600000 });
    });
}
...
if ($('#form_request').length) {
  ...
  function handleFileSelectClick(evt) {
    ...
    for (var i = 0, f; (f = files[i]); i++) {
      ...
      output.push(
        [
          '<li class="modal-block_checkload-item">',
          truncate(itemValue),
          '<span>&times</span></li>',
        ].join('')
      );
  ...
  document
    ...
    .addEventListener('change', handleFileSelectClick, false);
