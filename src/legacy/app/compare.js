    ...
    computed: {
      titleFromApi: function() {
        ...
        products.forEach(function(item) {
          ...
          ] = `до ${item['HeatTransferResistance']} м2°С/Вт`;
    ...
    methods: {
      ...
      changeButtonCompare: function() {
        ...
        if (this.list.length < 2) {
          $('.section.s5 .mbutton.blue.mbutton-block').addClass(
            'disable-compare-button'
          );
        } else {
          $('.section.s5 .mbutton.blue.mbutton-block').removeClass(
            'disable-compare-button'
          );
        }
      },
      toggleCompare: function(option) {
        if (
          ...
          $(
            '#fullpage_welcome .section.s5 .empty, #fullpage_welcome .section.s5 .sravnenie'
          ).toggle();
        } else if (option == 'right' && this.list.length > 1) {
          console.log('right');
          $(
            '#fullpage_welcome .section.s5 .empty, #fullpage_welcome .section.s5 .sravnenie'
          ).toggle();
      ...
      updateMenuList: function() {
        ...
        this.menuList = this.titleFromApi.slice().filter(item => {
          if (titleMap.indexOf(item.title) == -1) {
            return item;
          }
        });
      },
