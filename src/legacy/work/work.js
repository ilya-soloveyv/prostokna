...
if ($('.work').length) {
  ...
  $('.steps .steps__item').on('click', function () {
    $(this)
      ...
      .siblings()
      .removeClass('active')
      .closest('div.days-content')
      .find('div.step-content')
      .removeClass('active')
      .eq($(this).index())
      .addClass('active');
  });
  ...
  if ($(window).width() <= '992') {
    ...
    function swipeGo(swipe) {
      ...
      var step_active = $('.work .tabs-content.active .all-wrap .days-content')
        .eq(day_active_index)
        .find('.steps .steps__item.active');
