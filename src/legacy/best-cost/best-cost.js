import '@scss/best-cost/best-cost.scss';
...
if ($('#best-cost').length) {
  ...
  $('.cost').on('click', function () {
    setTimeout(() => {
      $('html,body').animate(
        { scrollTop: $('.background-form').offset().top + 200 + 'px' },
        { duration: 1e3 }
      );
    }, 250);
  });
  ...
  function createParticleCanvas() {
    ...
    if (window.innerWidth < 769)
      var cont = document.querySelector('.button-make-sale');
    if (window.innerWidth > 769)
      var cont = document.querySelector('.button-dekstop');
