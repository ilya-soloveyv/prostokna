...
import '@scss/product/products.scss';
  ...
  var productsMenu = new Vue({
    ...
    mounted: function() {
      Vue.set(
        this,
        'iProductID',
        Number(this.$el.getAttribute('data-iProductID'))
      );
    },
    watch: {
    ...
    methods: {
      clickBrand: function(brandItem) {
        this.materials.forEach((material, key1) => {
          material.brand.forEach((brand, key2) => {
            var status = false;
            if (
              brandItem.iBrandID == brand.iBrandID &&
              (brandItem.active == false || !brandItem.active)
            ) {
              status = true;
            }
        ...
        axios.get('/getProductMenu').then(response => {
          ...
          this.materials.forEach(material => {
            if (material.material_categories[0]) {
              Vue.set(
                material,
                'iMaterialCategoryID_active',
                material.material_categories[0].iMaterialCategoryID
              );
            } else {
      ...
      getFilterResult: function() {
        ...
        var iMaterialCategoryIDUnique = iMaterialCategoryID.filter(
          this.onlyUnique
        );
        ...
        if (this.filterParams.material[0]) {
          Vue.set(
            this.filterParams,
            'iMaterialID',
            this.filterParams.material[0].iMaterialID
          );
          if (this.filterParams.material[0].material_categories[0]) {
            Vue.set(
              this.filterParams,
              'iMaterialCategoryID',
              this.filterParams.material[0].material_categories[0]
                .iMaterialCategoryID
            );
          }
        }
