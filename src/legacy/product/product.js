  ...
  if ($('#product').length) {
    ...
    function positionTabBg(_this) {
      var _this = _this
        ...
        : $('#product .data .basic .info ul.nav li.nav-item a.active').parent();
    ...
    $('#product .data .basic .gallery .view2 .front')
      .stop()
      .animate({ width: '50%' }, 500);

    $('#product .data .basic .gallery .list2 .slick-change').on(
      'click',
      function (e) {
        let index = $(this).attr('data-slick-index');
        let iProductID = $(this).attr('data-iProductID');
        let iProductImageID = $(this).attr('data-iProductImageID');
        let sProductImageFrontName = $(this).attr(
          'data-sProductImageFrontName'
        );
    ...
    $(document)
      .on(
        'mouseenter',
        '#product .data .basic .gallery .view2 ul.point li',
        function () {
          $('#product .data .basic .gallery .view2 ul.point').css({
            'z-index': 100,
          });
        }
      )
    ...
    $('#product .data .basic .gallery .view2').click(function (e) {
      if (e.target.localName == 'img') {
        ...
        $(
          '#product .data .basic .gallery .list2 .slick .slick-list .slick-track .slick-slide'
        ).each(function (e, i) {
...
function useProductColorImage(index) {
  ...
  if (activeImg.length) {
    ...
    $('#product .data .colors .view').html(
      '<img src="/images/product/color/' +
        sProductColorFilename +
        '" data-index="' +
        index +
        '">'
    );
    $('#product .data .colors .list .colorName .bg span.sColorTitle').text(
      title
    );
    $('#product .data .colors .list .colorName .bg span.sColorTitleCode').text(
      code
    );
  }
}
...
function currentPoint2() {
  let image = $(
    '#product .data .basic .gallery .view2 .front .front_bg .bg img'
  );
  let iProductImageID = image.attr('data-iProductImageID');
  let points = $('#product .data .basic .gallery .view2 ul.point li');
  let points_current = $(
    '#product .data .basic .gallery .view2 ul.point li[data-iProductImageID=' +
      iProductImageID +
      ']'
  );
  points.stop().fadeOut();
...
function positionPoint2() {
  ...
  points.each(function (i, v) {
    ...
    let point_left =
      (view_width - image_width) / 2 + (image_width / 100) * point_x;
    let point_top =
      (view_height - image_height) / 2 + (image_height / 100) * point_y;
